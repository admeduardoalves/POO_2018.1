...
public class Usuario {
	...
	public String mostrarFeed(Chat c) {
		String saida="";
	    if(this.retornaChat(c)!=null) {
	    	quantidadedemensagens = c.contarMensagens(this.iduser);
		    saida+= c.getChat() + "(" + quantidadedemensagens +")";
	    }
	   
	   return saida;
	}

	public String mostrarNotific() {
		String saida="";
		for(Chat c : this.grups.getAll()) {
			quantidadedemensagens = c.contarMensagens(this.iduser);
			saida+= c.getChat() + "("+ quantidadedemensagens +")";
		}
		return saida;
